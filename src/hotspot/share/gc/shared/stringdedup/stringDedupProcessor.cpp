...
void StringDedup::Processor::cleanup_table(SuspendibleThreadSetJoiner* joiner,
                                           bool force) const {
  if (Table::cleanup_start_if_needed(force)) {
...
void StringDedup::Processor::cleanup_table_while_processing(SuspendibleThreadSetJoiner* joiner) const {
  if (Table::is_grow_needed()) {
    _cur_stat.report_process_pause();
    cleanup_table(joiner, false /* force */);
    _cur_stat.report_process_resume();
  }
}

class StringDedup::Processor::ProcessShared final : public ObjectClosure {
  SuspendibleThreadSetJoiner* _joiner;

public:
  ProcessShared(SuspendibleThreadSetJoiner* joiner) :
    _joiner(joiner)
  {}

  virtual void do_object(oop java_string) {
    if (_processor->yield_or_continue(_joiner, Stat::Phase::process)) {
      Table::add_shared_string(java_string);
      _processor->cleanup_table_while_processing(_joiner);
    }
  }
};

void StringDedup::Processor::process_shared(SuspendibleThreadSetJoiner* joiner) const {
  ProcessShared processor{joiner};
  StringTable::shared_oops_do(&processor);
}
...
public:
  ...
  virtual void do_oop(oop* ref) {
    if (_processor->yield_or_continue(_joiner, Stat::Phase::process)) {
      ...
      // Unlike other counters, only the processor thread accesses these,
      // because this is the only place where these are permitted rather
      // than being errors.
      if (java_string == nullptr) {
        ...
        if (value == nullptr) {
          // Request during String construction, before its value
...
void StringDedup::Processor::run_service() {
  ...
  for (bool shared = true; !should_terminate(); /* */) {
    ...
    cleanup_table(&sts_joiner, StringDeduplicationResizeALot);
    if (should_terminate()) break;
