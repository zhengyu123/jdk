...
// object keeps track of all the unique byte arrays used by deduplicated
// String objects.
//
...
private:
  ...
  // These are always written while locked, but may be read by the dedup
  // thread without lock.
  ...
  static Bucket* make_buckets(size_t number_of_buckets, int reserve = 0);
  static void free_buckets(Bucket* buckets, size_t number_of_buckets);

  static bool start_resizer(size_t number_of_entries);
...
public:
  ...
  // Install shared string's data array into the table.
  // Locks StringDedup_lock and table storage lock.
  static void add_shared_string(oop java_string);
  ...
  // Locks StringDedup_lock.
  // precondition: no cleanup is in progress.
  static bool cleanup_start_if_needed(bool force);
