...
public:
  Resizer(Bucket* buckets, size_t number_of_buckets) :
    _buckets(buckets),
...
StringDedup::Table::Bucket*
StringDedup::Table::make_buckets(size_t number_of_buckets, int reserve) {
...
void StringDedup::Table::add_shared_string(oop java_string) {
  typeArrayOop value = java_lang_String::value(java_string);
  assert(value != nullptr, "incomplete shared string");
  uint hash_code = compute_hash(value);
  // If no match for the shared string's backing array is found then add a
  // new entry to the table.  If a match is found, ignore the shared string.
  // All shared strings are processed before processing any requests, so
  // there won't be any collisions from that.  There could be a collision
  // between two shared strings with different coders but happening to have
  // arrays containing the same bytes.  In that case, we don't have a way to
  // choose which is better, so keep the one already in the table.
  if (find(value, hash_code).is_empty()) {
...
void StringDedup::Table::deduplicate(oop java_string) {
  ...
  typeArrayOop value = java_lang_String::value(java_string);
  assert(value != nullptr, "String value not initialized");
  uint hash_code = compute_hash(value);
...
bool StringDedup::Table::cleanup_start_if_needed(bool force) {
  ...
  if (force ||
      Config::should_grow_table(_number_of_buckets, adjusted) ||
      Config::should_shrink_table(_number_of_buckets, adjusted)) {
    // Resize if forced or needed with adjusted number of entries.
    return start_resizer(adjusted);
  } else if (Config::should_cleanup_table(_number_of_entries, dead_count)) {
    // Remove dead entries if needed.
...
bool StringDedup::Table::start_resizer(size_t number_of_entries) {
  ...
  _cleanup_state = new Resizer(_buckets, _number_of_buckets);
  int reserve = Bucket::needed_capacity(number_of_entries / new_size);
