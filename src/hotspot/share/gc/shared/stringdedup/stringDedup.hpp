...
// identical instances of String so that they share the same backing
// character array.
//
...
// different threads.
//
// The table uses entries from another weak OopStorage to hold the character
...
// when to cleanup dead entries in the table.  The usage pattern for the
// table is also very different from that of the request storages.  The
// request/processing storages are used in a way that supports bulk
...
// string has been added to the StringTable, its backing character array
// must not change.  Doing so would counteract C2 optimizations on string
// literals.  But an interned string might later become a deduplication
// candidate through the normal GC discovery mechanism.  This is handled by
// setting the no_deduplication flag in a string before interning it.  A
// string with that flag set may have its backing character array added to
// the deduplication table, but will not have its backing character array
// replaced by a different but equivalent array from the table.
//
