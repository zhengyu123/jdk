...
class StringDedup::Processor : public ConcurrentGCThread {
  ...
  void process_shared(SuspendibleThreadSetJoiner* joiner) const;
  void process_requests(SuspendibleThreadSetJoiner* joiner) const;

  void cleanup_table_while_processing(SuspendibleThreadSetJoiner* joiner) const;
  void cleanup_table(SuspendibleThreadSetJoiner* joiner, bool force) const;
